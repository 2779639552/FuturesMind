...
class TestProcessResearchReport:
    ...
    class _FakeLLM:
        """根据 prompt 区分两步:结构化提取(返回 JSON)→ 观点结论(返回 markdown)。"""
    ...
    def test_success_path(self, monkeypatch, tmp_path):
        ...
        rid = db.insert_research_report(
            variety="RB", title="华泰月报", source="华泰", filename="r.md", file_path=str(md_path)
        )
        ...
        assert got["direction"] == "看多"
        assert got["confidence"] == 0.85
        assert "核心观点" in (got["conclusion_md"] or "")
        structured = json.loads(got["structured_data"])
        assert structured["spot_price"]["value"] == 3200

        # 聚合 JSON 已同步写入
        agg = rd.load_research_data("RB")
        assert agg and agg["reports"][0]["id"] == rid
        rd._research_cache.clear()
